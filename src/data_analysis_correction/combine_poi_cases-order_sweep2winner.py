...
def _proc(
    ...
    for img_path in img_paths:
        ...
        available_subregs_cache: dict[int, list] = {}
        def _vert_mask(r):
            m = vert_mask_cache.get(r)
            if m is None:
                m = vert_nii.extract_label(r)
                vert_mask_cache[r] = m
            return m
        def _available_subregs(r):
            av = available_subregs_cache.get(r)
            if av is None:
                av = (subreg_nii * _vert_mask(r)).unique()
                available_subregs_cache[r] = av
            return av
        ...
        for r, s, c in list(primus.items()):
            ...
            avilable_subregs = _available_subregs(r)
