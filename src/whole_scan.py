...
def predict(subject, vert_msk_path, subreg_msk_path, model_path, dm_path, save_dir, gt_poi_path = None):
    ...
    for batch in dl:
        ...
        print(f"original_zoom: {original_zoom}")
        print(f"original_orientation: {original_orientation}")
        print(f"original_rotation: {original_rotation}")
        print(f"original_origin: {original_origin}")
        print(f"preprocessed_zoom: {preprocessed_zoom}")
        print(f"preprocessed_orientation: {preprocessed_orientation}")
        print(f"preprocessed_rotation: {preprocessed_rotation}")
        print(f"preprocessed_origin: {preprocessed_origin}")
        
        subject = batch["subject"][0]

        cutout_offset = batch["cutout_offset"].squeeze().detach().cpu().numpy()

        print(f"cutout_offset: {cutout_offset}")
        print(f"subject: {subject}\n\n\n\n\n")
        # Convert the predicted coordinates to a POI object
        
        unpadded_refined_preds_ctd = ev.np_to_ctd(
            pred_coords,
            vertebra=vertebra.item(),
            origin=preprocessed_origin,#original_origin, 
            rotation=preprocessed_rotation,#original_rotation,
            idx_list=poi_indices,
            shape=input_shape,
            zoom=preprocessed_zoom,#(1, 1, 1),
            offset=padding_offset,
            orientation=preprocessed_orientation#("L", "A", "S"),
        )
...
if __name__ == "__main__":
    ...
    save_dir = "/home/student/alissa/3dVertPois/src/predictions/whole_scan-test"
